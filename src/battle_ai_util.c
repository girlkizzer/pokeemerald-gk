...
// Functions
static bool32 AI_IsDoubleSpreadMove(enum BattlerId battlerAtk, u32 move)
{
    u32 numOfTargets = 0;
    ...
    for (enum BattlerId battlerDef = 0; battlerDef < MAX_BATTLERS_COUNT; battlerDef++)
    {
        ...
        if (!IsSemiInvulnerable(battlerDef, CHECK_ALL)
         || BreaksThroughSemiInvulnerablity(battlerAtk, battlerDef, move))
            numOfTargets++;
    }
...
static bool32 ShouldFallForIllusion(u32 illusionSpecies, enum BattlerId battlerId)
{
    ...
    for (u32 effectIndex = 0; effectIndex < additionalEffectCount; effectIndex++)
    {
        ...
        if (additionalEffect->self)
        {
            ...
            case MOVE_EFFECT_ALL_STATS_UP:
                ...
                {
                    if (BattlerStatCanRise(battlerAtk, statId))
                        return TRUE;
                }
                break;
            default:
...
s32 AI_WhoStrikesFirst(enum BattlerId battlerAI, enum BattlerId battler, enum Move aiMoveConsidered, enum Move playerMoveConsidered, enum ConsiderPriority considerPriority)
{
    ...
    if (AI_BATTLER_HAS_TRAIT(battlerAI, HOLD_EFFECT_LAGGING_TAIL) && !AI_BATTLER_HAS_TRAIT(battler, HOLD_EFFECT_LAGGING_TAIL))
        return AI_IS_SLOWER;
    else if (!AI_BATTLER_HAS_TRAIT(battlerAI, HOLD_EFFECT_LAGGING_TAIL) && AI_BATTLER_HAS_TRAIT(battler, HOLD_EFFECT_LAGGING_TAIL))
        return AI_IS_FASTER;
