...
#define OW_BERRY_MUTATION_CHANCE       50         // Determines the % chance of a mutation.
...
#define B_FLAG_FOLLOWERS_DISABLED      POKE_FOLLOWER_DISABLED          // Enables / Disables followers by using a flag. Helpful to disable followers for a period of time.
#define FLAG_FORCE_LOAD_OFFSCREEN_OBJEV   FORCELOAD_OFFSCREEEN   // Forces offscreen objects to stay loaded
#define FLAG_DONT_REMOVE_OFFSCREEN_OBJECT    TRUE  // Don't remove pushable boulder object
...
// Lighting
#define OW_SHADOW_INTENSITY             2       // Ranges from 0 to 16, where 0 is fully transparent and 16 is black.
...
#define OW_FLAG_PAUSE_TIME          PAUSE_TIME  // If this flag is set and OW_USE_FAKE_RTC is enabled, seconds on the in-game clock will not advance.
#define OW_FLAG_NO_ENCOUNTER        NO_ENCOUNTER  // If this flag is set, wild encounters will be disabled.
#define OW_FLAG_NO_TRAINER_SEE      TRAINER_NOT_SEE  // If this flag is set, trainers will not battle the player unless they're talked to.
#define OW_FLAG_NO_COLLISION        NO_COLLISION  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
#define OW_FLAG_POKE_RIDER          POKE_RIDER  // If this flag is set, the player will be able to use fly from the Pokenav Region Map and the Town Map key item by pressing 'R' on a city/location they are able to fly to.
