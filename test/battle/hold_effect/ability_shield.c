...
#if MAX_MON_TRAITS > 1
DOUBLE_BATTLE_TEST("Ability Shield prevents Intimidate from reactivating after Neutralizing Gas ends (Traits)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(5); }
        PLAYER(SPECIES_WYNAUT) { Speed(4); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); HP(1); Speed(1); }
        OPPONENT(SPECIES_GYARADOS) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_INTIMIDATE); Item(ITEM_ABILITY_SHIELD); Speed(3); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        ABILITY_POPUP(opponentLeft, ABILITY_NEUTRALIZING_GAS);
...
#if MAX_MON_ITEMS > 1
DOUBLE_BATTLE_TEST("Ability Shield prevents Intimidate from reactivating after Neutralizing Gas ends (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { Speed(5); }
        PLAYER(SPECIES_WYNAUT) { Speed(4); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); HP(1); Speed(1); }
        OPPONENT(SPECIES_GYARADOS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_PECHA_BERRY, ITEM_ABILITY_SHIELD); Speed(3); }
    } WHEN {
        TURN { MOVE(playerLeft, MOVE_SCRATCH, target: opponentLeft); }
    } SCENE {
        ABILITY_POPUP(opponentLeft, ABILITY_NEUTRALIZING_GAS);
...
SINGLE_BATTLE_TEST("Ability Shield protects against Neutralizing Gas (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mold Breaker (no message) (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield protects against Mycelium Might (no message) (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield protects against Sunsteel Strike (no message) (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield activates a previously suppressed ability when obtained (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TRICK) == EFFECT_TRICK);
        PLAYER(SPECIES_GYARADOS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_PECHA_BERRY, ITEM_POTION); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); Items(ITEM_PECHA_BERRY, ITEM_ABILITY_SHIELD); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); MOVE(opponent, MOVE_TRICK); }
        TURN { MOVE(opponent, MOVE_TRICK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield doesn't reactivate an ability when receiving if user already had an Ability Shield (Items)")
{

    GIVEN {
        ASSUME(GetMoveEffect(MOVE_TRICK) == EFFECT_TRICK);
        PLAYER(SPECIES_GYARADOS) { Ability(ABILITY_INTIMIDATE); Items(ITEM_PECHA_BERRY, ITEM_ABILITY_SHIELD); }
        OPPONENT(SPECIES_KOFFING) { Ability(ABILITY_NEUTRALIZING_GAS); Items(ITEM_PECHA_BERRY, ITEM_ABILITY_SHIELD); }
    } WHEN {
        TURN { MOVE(player, MOVE_TRICK); }
    } SCENE {
...
SINGLE_BATTLE_TEST("Ability Shield protects the user from having its ability suppressed by Gastro Acid (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield doesn't protect the user's ability from being suppressed by Gastro Acid (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_GASTRO_ACID) == EFFECT_GASTRO_ACID);
        ASSUME(GetMoveEffect(MOVE_TRICK) == EFFECT_TRICK);
        PLAYER(SPECIES_BLAZIKEN) { Ability(ABILITY_SPEED_BOOST); Items(ITEM_PECHA_BERRY, ITEM_POTION); }
        OPPONENT(SPECIES_WOBBUFFET) { Items(ITEM_PECHA_BERRY, ITEM_ABILITY_SHIELD); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_GASTRO_ACID); MOVE(player, MOVE_TRICK); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_GASTRO_ACID, opponent);
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap (Items)")
{
    enum Item item;
...
SINGLE_BATTLE_TEST("Ability Shield protects against Skill Swap even if user has Klutz (Items)")
{
    enum Item item;
...
// These currently do not activate, but probably should do held item animation + message
TO_DO_BATTLE_TEST("Ability Shield prevents the user's Trace from changing its ability (Items)");
TO_DO_BATTLE_TEST("Ability Shield prevents the user's Receiver from changing its ability (Items)");
TO_DO_BATTLE_TEST("Ability Shield protects against Wandering Spirit (Items)");
TO_DO_BATTLE_TEST("Ability Shield protects against Mummy/Lingering Aroma (Items)");
#endif
