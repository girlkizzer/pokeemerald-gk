...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Booster Energy will activate Quark Drive after Electric Terrain ends (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Booster Energy will activate Protosynthesis after harsh sunlight ends (Items)")
{
    GIVEN {
        WITH_CONFIG(B_ABILITY_WEATHER, GEN_6);
...
SINGLE_BATTLE_TEST("Booster Energy's Protosynthesis boost is preserved when weather changes (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Booster Energy activates Protosynthesis and increases highest stat (Items)")
{
    u32 attack, defense, speed, spAttack, spDefense;
...
SINGLE_BATTLE_TEST("Booster Energy activates Quark Drive and increases highest stat (Items)")
{
    u32 attack, defense, speed, spAttack, spDefense;
...
SINGLE_BATTLE_TEST("Booster Energy's Quark Drive boost is preserved when terrain changes (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Booster Energy increases special attack by 30% if it is the highest stat (Items)", s16 damage)
{
    u32 species;
    enum Ability ability;
    enum Item item;
...
SINGLE_BATTLE_TEST("Booster Energy increases special defense by 30% if it is the highest stat (Items)", s16 damage)
{
    u32 species;
    enum Ability ability;
    enum Item item;
...
SINGLE_BATTLE_TEST("Booster Energy can't be tricked if a Paradox species is involved (Items)")
{
    GIVEN {
        ASSUME(gSpeciesInfo[SPECIES_IRON_MOTH].isParadox == TRUE);
...
DOUBLE_BATTLE_TEST("Booster Energy triggers correctly for all battlers if multiple fainted the previous turn (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_CATERPIE) { HP(1); }
...
DOUBLE_BATTLE_TEST("Booster Energy activates on any terrain (Items)")
{
    GIVEN {
...
DOUBLE_BATTLE_TEST("Booster Energy activates on air locked sun (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
...
DOUBLE_BATTLE_TEST("Booster Energy will not activate on terrain if user has Protosynthesis instead of Quark Drive (Items)")
{
    GIVEN {
