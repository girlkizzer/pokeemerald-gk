...
SINGLE_BATTLE_TEST("Liquid Ooze causes Absorb users to lose HP instead of heal (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TENTACOOL) { Ability(ABILITY_CLEAR_BODY); Innates(ABILITY_LIQUID_OOZE); }
    } WHEN {
        TURN { MOVE(player, MOVE_ABSORB); }
    } SCENE {
        ANIMATION(ANIM_TYPE_MOVE, MOVE_ABSORB, player);
...
SINGLE_BATTLE_TEST("Liquid Ooze causes Strength Sap users to lose HP instead of heal (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_WOBBUFFET) { Attack(atkStat); Ability(ABILITY_SHADOW_TAG); Innates(ABILITY_LIQUID_OOZE); }
    } WHEN {
        TURN { MOVE(player, MOVE_STRENGTH_SAP); if (atkStat == 490) { SEND_OUT(player, 1); } }
    } SCENE {
        MESSAGE("Wobbuffet used Strength Sap!");
...
SINGLE_BATTLE_TEST("Liquid Ooze causes leech seed victim to faint before seeder (Traits)")
{
    ...
    GIVEN {
        PLAYER(SPECIES_BULBASAUR)   { HP(1); }
        OPPONENT(SPECIES_TENTACOOL) { HP(1); Ability(ABILITY_CLEAR_BODY); Innates(ability); }
    } WHEN {
        TURN { MOVE(player, MOVE_LEECH_SEED); }
    } SCENE {
        // Player seeds opponent
...
SINGLE_BATTLE_TEST("Liquid Ooze causes Dream Eater users to lose HP instead of heal (Gen 5+) (Traits)")
{
    ...
    GIVEN {
        ...
        ASSUME(GetMoveEffect(MOVE_DREAM_EATER) == EFFECT_DREAM_EATER);
        PLAYER(SPECIES_WOBBUFFET);
        OPPONENT(SPECIES_TENTACRUEL) { Ability(ABILITY_CLEAR_BODY); Innates(ABILITY_LIQUID_OOZE); }
    } WHEN {
        TURN { MOVE(opponent, MOVE_SCRATCH); MOVE(player, MOVE_SPORE); }
        TURN { MOVE(player, MOVE_DREAM_EATER); }
    } SCENE {
