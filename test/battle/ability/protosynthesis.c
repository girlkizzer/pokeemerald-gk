...
SINGLE_BATTLE_TEST("Protosynthesis uses Wonder Room swapped defenses when choosing boosted stat (Traits)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_WONDER_ROOM) == EFFECT_WONDER_ROOM);
        PLAYER(SPECIES_ROARING_MOON) { Ability(ABILITY_LIGHT_METAL); Innates(ABILITY_PROTOSYNTHESIS); Attack(50); Defense(200); SpAttack(40); SpDefense(60); Speed(70); Moves(MOVE_WONDER_ROOM); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_SUNNY_DAY); Speed(60); }
    } WHEN {
        TURN { MOVE(player, MOVE_WONDER_ROOM); MOVE(opponent, MOVE_SUNNY_DAY); }
    } SCENE {
...
#if MAX_MON_ITEMS > 1
SINGLE_BATTLE_TEST("Protosynthesis activates in Sun before Booster Energy (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
...
SINGLE_BATTLE_TEST("Protosynthesis doesn't activate for a transformed battler (Items)")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET);
        PLAYER(SPECIES_GREAT_TUSK) { Ability(ABILITY_PROTOSYNTHESIS); Items(ITEM_PECHA_BERRY, ITEM_BOOSTER_ENERGY); }
...
SINGLE_BATTLE_TEST("Protosynthesis activates even if the Pokémon is holding an Utility Umbrella (Items)")
{
    GIVEN {
...
SINGLE_BATTLE_TEST("Protosynthesis damage calculation is correct (Items)")
{
    s16 dmg;
    s16 expectedDamage;
